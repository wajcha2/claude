...
2026-09-24 19:24 UTC: the first run (6 components, logs `_s<k>.log`) used the original greedy contraction order of
`hwv.contract`; a shape-only simulation of that order predicted intermediates of 10-60 GB on many pass-1 fillings
(the batch of highest-weight vectors times up to 7^7 word indices), i.e. certain OOM kills. `hwv.py` now (a) picks the
contraction path with opt_einsum's dynamic programming minimising the largest intermediate (`pip install opt_einsum`;
`PATHOPT=greedy` restores the old order) and (b) halves the row or column batch recursively whenever the predicted
largest intermediate exceeds `MEMCAP` int64 elements (default 4e7 = 0.32 GB). Results are bit-identical to the old
code (18 random cases incl. forced splits, d=4 sweep, C^3 d=5 test) and typically faster. All runs from `_r1` on use it.
...
|---|---|---|---|
| 1 | MAXDIM=150 | 279 | running since 2026-09-24 19:09 UTC (restarted 19:24 UTC with the new hwv.py, logs `_r1`) |
